...
public class University {
    ...
    private static HashMap<String, Dosen> listDosen= new HashMap<>();
    private static HashMap<String, Mahasiswa> listMahasiswa= new HashMap<>();
    private static HashMap<String, Matakuliah> listMatakuliah = new HashMap<>();
    ...
    public static void setListDosen(){
        ...
        listDosen.put("T701",antik);
        listDosen.put("T808",cahya);
        listDosen.put("T904",Mikha);
    ...
    public static  void setListMahasiswa(){
        ...
        List<Enrollment> enroldesy = new LinkedList<>(Arrays.asList(
                new Enrollment(listMatakuliah.get("NetFun"),LocalDate.of(2018,4,12)),
                new Enrollment(listMatakuliah.get("DatStr"),LocalDate.of(2018,8,18))
        ));
        ...
        listMahasiswa.put("A021",alex);
        listMahasiswa.put("A022",Desy);
        listMahasiswa.put("A023",Joko);


    }

    public static void printInfoUniversity(String nama, LocalDate tglberdiri, String kota, String negara){
        System.out.println(String.format("University %s\nDibangun pada %s\ndi %s , %s",University.nama=nama,University.tglberdiri = tglberdiri
                ,University.kota = kota,University.Negara=negara));
    }

    public static void Insialisasi(){
        printInfoUniversity("Unicorn",LocalDate.of(2022,12,12),"GoldCod","Australia");
        setListMatakuliah();
        setListMahasiswa();
        setListDosen();
    }
    ...
    public static void showMahasiswa(){
        ...
        for (Map.Entry mhs: listMahasiswa.entrySet()
             ) {
            Mahasiswa mahasiswa = (Mahasiswa) mhs.getValue();
            System.out.println(String.format( "NIK : %s, Nama : %s %s",mahasiswa.getNIKmhs(),mahasiswa.getNamaDpn(),mahasiswa.getNamaBlkng()));
        }
        ...
        do{
            ...
            try{
                pilih = inp.nextLine();
                check = true;
                if(!(Integer.parseInt(pilih)>0 && Integer.parseInt(pilih)<4) ){
        ...
        switch (Integer.parseInt(pilih)){
            case 1:{
                System.out.println("Masukin mahasiswa yang ingin ditampilkan detail ");
                String nik = inp.nextLine();

                Mahasiswa mhs = listMahasiswa.get(nik);
                mhs.printDetailMahasiswa();
                break;}
            case 2:{
                Main.menu();
                break;}
